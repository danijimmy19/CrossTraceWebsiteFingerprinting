...
python m-2-tfidf-cosine.py /path/to/wt-def-model.h5 /path/to/paired/data

/path/to/wt-def-model.h5 is the path to CNN model trained on Walkie-Talkie defended dataset
...
CNN_PATH = sys.argv[1]
# path of the dataset
# data_dir = '../webfp-crossed-fingerprints-data/closed-world/wt-def-keyword/wt-analysis-four-traces-250/paired-data/wt-def-keyword-2-paired.csv'
data_dir = sys.argv[2]
...
def get_similarity(l1, l2):
    ...
    # Generate matrix of words
    tfidf_matrix = vectorizer.fit_transform(documents)
    vecs = pd.DataFrame(tfidf_matrix.toarray(),columns=vectorizer.get_feature_names(),index=['Document 0','Document 1'])
    score = cosine_similarity(tfidf_matrix, tfidf_matrix)
    sim = score[0][1]
#         print(score)
...
#         print('processing pair %d - %s'%(i, temp))
        ...
        if k3 in pre_dist:
            ...
            d3 = get_similarity(t3, t4)
            pre_dist[k3] = d3

        total_dist = round((d1**2) + (d2**2) + (d3**2), 5)
...
def get_accuracy(pairs_c, pairs_c_dist, t1_actual_label, t2_actual_label, t3_actual_label, t4_actual_label):
    '''
    ...
    for p in range(len(dist)):
        ...
        if (temp_pairs[0] not in sel_pairs_tr_1) and (temp_pairs[1] not in sel_pairs_tr_2) and (temp_pairs[2] not in sel_pairs_tr_3) and (temp_pairs[3] not in sel_pairs_tr_4):
            sel_pairs_tr_1.append(temp_pairs[0])
            sel_pairs_tr_2.append(temp_pairs[1])
            sel_pairs_tr_3.append(temp_pairs[2])
            sel_pairs_tr_4.append(temp_pairs[3])
            ...
            if (t3_actual_label in sel_pairs_tr_3):
                count_3 += 1
            if (t4_actual_label in sel_pairs_tr_4):
                count_4 += 1

            accuracy = round((count_1 + count_2 + count_3 + count_4)/4.0, 4)
...
#     for i in range(0, 8, 4):
        ...
        t4 = x_test[i+3].reshape(-1, params['features'], 1)
        t4_actual_label = list(y_test[i+3]).index(1)
        print('actual label of trace t4: ', t4_actual_label)
        # making predictions for trace t1
        t4_pred = model.predict(t4)
        top_list_4 = get_top_n(t4_pred, top_n)

        # generate n^3 unique combinations, where n is number of predictions
        print('+'*60)
        all_preds = [top_list_1, top_list_2, top_list_3, top_list_4]
        pairs_c, pairs_c_dist = get_dist(all_preds)
        print('+'*60)

        # getting the accuracy
        temp_acc_1, temp_acc_2, temp_acc_3, temp_acc_4 = get_accuracy(pairs_c, pairs_c_dist, t1_actual_label, t2_actual_label, t3_actual_label, t4_actual_label)
        acc_1.append(temp_acc_1)
        acc_2.append(temp_acc_2)
        acc_3.append(temp_acc_3)
        acc_4.append(temp_acc_4)
        
    return acc_1, acc_2, acc_3, acc_4


acc_1, acc_2, acc_3, acc_4 = evaluation()


acc_1_t = [item for sublist in acc_1 for item in sublist]
acc_2_t = [item for sublist in acc_2 for item in sublist]
acc_3_t = [item for sublist in acc_3 for item in sublist]
acc_4_t = [item for sublist in acc_4 for item in sublist]


print('Average Top-1 accuracy: ', np.mean(acc_1_t))
print('Average Top-2 accuracy: ', np.mean(acc_2_t))
print('Average Top-3 accuracy: ', np.mean(acc_3_t))
print('Average Top-4 accuracy: ', np.mean(acc_4_t))