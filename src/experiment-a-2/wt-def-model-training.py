...
python wt-def-model-training.py /path/to/save/model/model-name.h5 /path/to/wt-defended/dataset
...
# path to save the model trained on WT defended dataset
MODEL_PATH = sys.argv[1]
# path to wt-defended dataset to be used for training
...
x_train, y_train, x_valid, y_valid, x_test, y_test, nb_classes = load_data_wt_def(DATA_PATH, p["features"])
...
# history_df.to_csv(MODEL_PATH + 'no-def-alexa-100-history.csv',index=False)
# saving the plot of the model
# plot_model(model, to_file=MODEL_PATH + 'no-def-alexa-100.png',
